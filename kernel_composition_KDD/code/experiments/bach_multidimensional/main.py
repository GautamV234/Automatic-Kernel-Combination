...
# Importing libraries
##############################
from kernel_composition_KDD.code.model.model_4 import Best_model_params, Inference, KernelCombinations
import numpy as np
import torch
...
class covar_kernels():
    def __init__(self, kernel_list:list[str], num_features:int,device):
...
if __name__ == '__main__':
    ...
    # Defining paths
    #####################################
    root_path = '/home/gautam.pv/nlim/kernel_composition_KDD/code/experiments/bach_multidimensional'
    data_path = '/home/gautam.pv/nlim/kernel_composition_KDD/data/bach_synth_r_200.mat'
    yaml_file = '/home/gautam.pv/nlim/kernel_composition_KDD/yaml/bach_multi_dim_exp.yaml'
    ######################################
    # Data Loading
    ######################################
        ...
        for id, specs  in yaml_data.items():
            ...
            if heuristic:
                ...
                for degree_val, combination_list in zip(degrees_, combination_lists):
                    base_kernels_map = covar_kernels(kernel_list = kernels_yaml, num_features = X_train.shape[1],device=device).get_base_kernels()
