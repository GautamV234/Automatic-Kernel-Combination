...
# Implement for multiple features (Done)
# Implement multiple random initializations (Doing)
############################################################
...
class KernelCombinations():
                ...
                if (str(self.heuristic_idx) not in kernel_idx_list):
                    ...
                    for kernel_comb, kernel_comb_name in zip(unique_kernel_combinations, unique_kernel_combination_names):
                        for initialisation_num in range(1, self.num_initializations+1):
                            ...
                            #####################
                            # Random Initializations for Kernel
                            #####################
                            init_params = {}
                            for param_name, param in kernel_comb.named_parameters():
                                param_name: str
                                init_params[param_name] = torch.rand(1).to(self.device)
                            kernel_comb.initialize(**init_params)
    ...
    def fit_kernel_combinations(self, plot_fit=False, specific_degree=None, log_file=None):
        ...
        self.kernel_encoding_keys_iterable = "".join(
            str(key) for key in self.kernel_encoding.keys())
        print(f"self.kernel_encoding_keys_iterable: {self.kernel_encoding_keys_iterable}")
