...
class Best_model_params():
    ...
    combination: GPy.kern.src.kern.Kern
...
class Inference():
    ...
    def __init__(self, x_train, y_train, x_test, y_test, best_model_params: Best_model_params, scores_train: Scores = None, degree=1, device='cpu'):
        ...
        self.model:GPy.models.gp_regression.GPRegression
        self.likelihood = None
        self.degree = degree
        self.device = device
        self.Y_pred,self.Y_var = self.model.predict(self.X_test)
        
        self.scores_test = None
    

    def plot_fit_gpy(self,figure_save_path):
        """"
        ...
        sort_idx = np.argsort(self.X_test[:, 0])
        self.X_test = self.X_test[sort_idx]
        self.Y_test = self.Y_test[sort_idx]
        self.Y_pred = self.Y_pred[sort_idx]
        self.Y_var = self.Y_var[sort_idx]
        fig = plt.figure(figsize=(8, 6), dpi=100)
        ax = fig.add_subplot(1, 1, 1)
        ax.scatter(self.X_train, self.Y_train, label="Training Data")
        ax.scatter(self.X_test, self.Y_test, label="Ground Truth")
        ax.plot(self.X_test, self.Y_pred, label="Model Fit")
        ax.fill_between(self.X_test[:, 0], self.Y_pred[:, 0] - 2*np.sqrt(self.Y_var[:, 0]), 
                 self.Y_pred[:, 0] + 2*np.sqrt(self.Y_var[:, 0]), alpha=0.2, label="Uncertainty")
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.legend()
    ...
    def tabulated_scores_gpy(self,metrics_log_file=None):
        # calculate the negative log predictive density (NLPD)
        log_likelihoods = []
        for y_pred, y_var, y_true in zip(self.Y_pred, self.Y_var, self.Y_test):
            l = -0.5 * np.log(2 * np.pi * y_var) - 0.5 * ((y_true - y_pred)**2) / y_var
            log_likelihoods.append(l)
        lpd = np.sum(log_likelihoods)
        nlpd = -lpd / self.Y_test.size
        print("Negative Log Predictive Density (NLPD):", nlpd)
        # calculate the root mean squared error (RMSE)
        rmse = np.sqrt(np.mean((self.Y_pred - self.Y_test)**2))
        print("Root Mean Squared Error (RMSE):", rmse)
        # use tabulate to print the results
        table = tabulate([["NLPD", nlpd], ["RMSE", rmse]], headers=["Metric", "Value"])
        scores_dict = {"Negative Log Predictive Density": nlpd, "Root Mean Squared Error": rmse}
        print(table, file=metrics_log_file)
        for param_name,param_value in zip(self.model.parameter_names(),self.model.param_array):
            print(f"{param_name} : {param_value}", file=metrics_log_file)
        return scores_dict
...
class KernelCombinations():
    def __init__(self, degree, kernel_map: dict, X_train, Y_train, X_test, Y_test, X_val=None, Y_val=None, **kwargs):
        ...
        self.num_initializations = kwargs.get("num_initializations", 4)
    ...
    def fit_heuristic_kernel_combinations(self, plot_fit=False, heuristic_kernel: dict = None, specific_degree=None, log_file=None):
        ...
        self.kernel_encoding_keys_iterable = [
            str(key) for key in self.kernel_encoding.keys()]
                ...
                if (str(self.heuristic_idx) not in kernel_idx_list):
                    ...
                    for kernel_comb, kernel_comb_name in zip(unique_kernel_combinations, unique_kernel_combination_names):
                        m = GPy.models.GPRegression(
                            self.X_train, self.Y_train, kernel_comb)
                        print(f"Model being trained: {kernel_comb_name}")
                        try:
                            m.optimize_restarts(
                                num_restarts=self.num_initializations)
                        except Exception as e:
                            print("Error in training model,skipping...") 
                            continue                           
                        training_loss = m.objective_function()
                        new_model_characteristics = Model_characteristics(model=m, likelihood=None, combination_name=kernel_comb_name,
                                                                          combination=kernel_comb, epoch_number=None, initialization_number=self.num_initializations, training_loss=training_loss)
    ...
    def fit_kernel_combinations(self, plot_fit=False, specific_degree=None, log_file=None):
        ...
        for idx, kernel_idx_list in enumerate(iter):
            ...
            for kernel_comb, kernel_comb_name in zip(unique_kernel_combinations, unique_kernel_combination_names):
                ...
                print(f"Model being trained: {kernel_comb_name}")
                try:
                    m.optimize_restarts(num_restarts=self.num_initializations)
                except Exception as e:
                    print("Error in training model, skipping...")
                    continue
                    # print(e)
                    # raise Exception("Model Training Failed for kernel combination: ", kernel_comb_name)
                training_loss = m.objective_function()
                new_model_characteristics = Model_characteristics(model=m, likelihood=None, combination_name=kernel_comb_name,combination=kernel_comb, epoch_number=None, initialization_number=self.num_initializations, training_loss=training_loss)
                model_characteristics.append(new_model_characteristics)
                ########################################################
        for new_model_characteristics in model_characteristics:
    ...
    def main(self, heuristic=False, combination_list=None, log_file=None, num_epochs=100, lrate=0.1):
        ...
        if heuristic:
            ...
            _, _, best_model_params, model_tracker = self.fit_kernel_combinations(
